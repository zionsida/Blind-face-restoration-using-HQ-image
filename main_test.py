...
def optimal_reference_selection(lq_landmarks, hq_landmark_paths, lq_path):
    # read the landmarks of all reference images
    RefPaths = os.listdir(hq_landmark_paths)
    
    lq_path=lq_path.split('/')[-1]+'.txt'
    
    if lq_path in RefPaths:
        RefPaths.remove(lq_path)
...
if __name__ == '__main__':
    ...
    weights = torch.load('/home/undergrad_te/siondijun/ASFFNet512/checkpoints/ASFFNet512.pth') 
    ...
    for size_text in ["80"]:
        path_dir = '/home/undergrad_te/siondijun/Testlists_'+size_text
        save_dir='/home/undergrad_te/siondijun/ASFFNetResult_'+size_text+'+guid'
        save_dir2='/home/undergrad_te/siondijun/ASFFNetResult_'+size_text
        ...
        for dataforder in tqdm(file_list):
            #print(dataforder)
            testlistdir=path_dir+'/'+dataforder+'/TestLists.txt'
            parser = argparse.ArgumentParser()
            parser.add_argument('-s', '--img_list', type=str, default=testlistdir, help='input path of lq image')
            parser.add_argument('-d', '--out_path', type=str, default=save_dir+"/"+dataforder, help='save path of restoration result')
            ...
            for line in lines:
                ...
                optimal_ref_landmarks, optimal_ref_path = optimal_reference_selection(lq_landmarks, land_path,lq_path)
                ...
                #format(osp.basename(lq_path))
            
                os.makedirs(save_dir2+'/'+dataforder, exist_ok=True)
                cv2.imwrite(save_dir2+'/'+dataforder+'/'+format(osp.basename(lq_path)),save_out)
